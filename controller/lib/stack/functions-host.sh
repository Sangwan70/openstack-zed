# This file contains bash functions that are used by stack on the host.
...
# STACK=exec_cmd
#
# Execute command only if STACK is set:
# ${STACK:-:} cmd args
#
# Execute command only if STACK is not set:
# ${STACK:+:} cmd args
#
...
function keyboard_send_string {
    ...
    while IFS=  read -r -n1 char; do
        ...
        if [[ $((cnt % 50)) -eq 0 ]]; then
            STACK= ${WBATCH:-:} conditional_sleep 1
        fi
        cnt=$((cnt + 1))
...
# Check permission for stack insecure private key
function check_stack_private_key {
    local key_name="stack_key"
    local stack_key_dir=$LIB_DIR/stack-ssh-keys
    local stack_key_path=$stack_key_dir/$key_name

    if ! ls -l "$stack_key_path"|grep -q "^-r--------"; then
        echo "Adjusting permissions for $stack_key_path"
        chmod 400 "$stack_key_path"
    fi
}
...
function vm_scp_to_vm {
    ...
    while (($#)); do
        ...
        scp -q -r \
            -i "$LIB_DIR/stack-ssh-keys/stack_key" \
...
# autostart <file> [<file> ...]
# e.g. autostart zero_empty.sh stack/base_fixups.sh
function autostart {
...
function check_md5 {
    ...
    if $md5exe "$file" | grep -q "$csum"; then
        ...
        echo >&2 "$file"
        echo -e >&2 "${CError:-}Please remove file and re-run stack script.${CReset:-}"
        exit 1
    fi
}
