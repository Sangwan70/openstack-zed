...
function vm_install_base {
    ...
    ${STACK:-:} check_md5 "$INSTALL_ISO" "$ISO_MD5"
    ...
    # For wbatch, install stackauto as a boot service
    ${WBATCH:-:} autostart stack/activate_autostart.sh

    autostart stack/base_fixups.sh

    # By default, set by lib/stack/lib.* to something like scripts.ubuntu_base
    ...
    # (for wbatch, stackauto does the processing instead)
    ...
    ${STACK:-:} mv -vf "$base_build_disk" "$base_disk_path"
    ${WBATCH:-:} wbatch_rename_disk "$base_build_disk" "$base_disk_path"

    ${WBATCH:-:} wbatch_end_file

    echo >&2 -e "${CData:-}$(date) ${CStatus:-}stack vm_install ends\n${CReset:-}"
}
